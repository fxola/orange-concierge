    ...
    >
      ...
      {items.length === 0 ? (
        ...
        <div className="divide-y divide-border/70">
          {items.map((item) => {
            const exactMatch = item.evidence
              ? isExactTranscriptMatch(item.value, item.evidence.quote)
              : false;
            return (
              <div key={item.path} className="grid gap-3 px-4 py-3.5">
                ...
                {item.evidence ? (
                  exactMatch ? (
                    <p className="inline-flex w-fit items-center gap-1.5 rounded-full bg-success-surface px-2.5 py-1 text-xs font-medium text-success">
